...
from PySide6.QtCore import QObject, Property, Signal, Slot, QTranslator, QLocale
...
class AppTranslator(QObject):
    ...
    def setLanguage(self, locale_name: str):  # sample: zh_CN; en_US
        ...
        old_rinui_translator = self.rinui_translator
        self.app.removeTranslator(old_translator)
        if old_rinui_translator:
            self.app.removeTranslator(old_rinui_translator)
        ...
        # Qt queries translators in reverse installation order. Keep application
        # translations ahead of RinUI's fallback catalog for matching messages.
